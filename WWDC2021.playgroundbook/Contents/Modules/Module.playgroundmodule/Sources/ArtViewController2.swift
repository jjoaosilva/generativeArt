...
public class ArtViewController2: UIViewController {
    ...
    public var numRosesParameter: Int = 20
    ...
    private var numRosesParameterDefault: Int = 20

    lazy var scene: ArtScene2 = {
        let scene = ArtScene2()
        ...
        scene.numRosesParameter = self.colorsParameter.count > 0 ? self.numRosesParameter : self.numRosesParameterDefault
        return scene
    }()
...
public func CreateLiveView() -> PlaygroundLiveViewable{
    let view = SKView()
    let scene = ArtScene2()
    scene.scaleMode = .resizeFill
